...
Using the version number reported in the previous command, set the following environment variable in your Terminal's shell accordingly, e.g., in the bash shell: 
```
$ export PHYSICELL_CPP=/usr/local/bin/g++-9
```
and the Makefile will use it. You should permanently set this in your environment via: 

`$ echo export PHYSICELL_CPP=g++-9 >> ~/.bash_profile`.
